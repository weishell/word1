...
![image-20200209043049360](.\images\目录结构.png)
...
![image-20200225220457313](.\images\image-20200225220457313.png)
...
![image-20200210033341264](.\images\状态配置文件.png)
...
要手动断开节点，请从节点的行中选择"断开连接"图标(![断开图标](.\images\关机图标.png))。

![img](.\images\集群管理页面2-断开节点.png)

断开连接的节点可以连接(![连接图标](.\images\连接图标.png))，卸载(![卸载图标](.\images\卸载图标.png))或删除(![删除图标](.\images\删除图标.png))。

![img](.\images\说明.png)并非所有处于"已断开连接"状态的节点都可以卸载。如果节点断开连接且无法访问，则节点无法接收卸载请求以启动卸载。此外，由于防火墙规则，可能会中断或阻止卸载。



#### 卸载节点

保留在断开连接的节点上的流文件可以通过卸载重新平衡到集群中的其他活动节点。在Cluster Management对话框中，为Disconnected节点选择"Offload"图标(![卸载图标](.\images\卸载图标.png))。这将停止所有处理器，终止所有处理器，停止在所有远程进程组上传输，并将流文件重新平衡到集群中的其他连接节点。

![img](.\images\集群管理页面3-卸载节点.png)

由于遇到错误(内存不足，没有网络连接等)而保持"卸载"状态的节点可以通过重新启动节点上的NiFi重新连接到集群。卸载的节点可以重新连接到集群(通过选择连接或重新启动节点上的NiFi)或从集群中删除。

![img](.\images\集群管理页面4-重启的节点.png)

#### 删除节点

在某些情况下，DFM可能希望继续对流进行更改，即使节点未连接到集群也是如此。在这种情况下，DFM可以选择完全从集群中删除节点。在Cluster Management对话框中，为Disconnected或Offloaded节点选择"Delete"图标(![删除图标](.\images\删除图标.png))。删除后，在重新启动节点之前，节点无法重新加入集群。
...
1. **创建GenerateFlowFile并设置大小**
   ...
   ![设置GenerateFlowFile大小](./images/设置GenerateFlowFile大小.gif)

   

2. **配置FlowFile**

   ![设置GenerateFlowFile定时](./images/设置GenerateFlowFile定时.gif)
...
3. **创建ReplaceText并连接**
   ...
   ![创建连接ReplaceText](./images/创建连接ReplaceText.gif)

   

4. **启动GenerateFile**

   ![启动GenerateFlowFile](./images/启动GenerateFlowFile.gif)

   

5. **配置ReplaceText**

![配置ReplaceText](./images/配置ReplaceText.gif)

**6.创建PutFile**

停止GenerateFlowFile，然后创建PutFile。

![创建PutFile](./images/创建PutFile.gif)
...
#### ![导入模板](./images/导入模板.gif)2.查看模板列表

![模板列表](./images/模板列表.gif)

#### 3.创建模板引用

![引用模板](./images/引用模板.gif)
...
![移动处理器到组](./images/移动处理器到组.gif)

#### 3.进入与退出组

![进入与退出组](./images/进入与退出组.gif)

#### 4.创建模板

![创建模板](./images/创建模板.gif)



#### 5.下载模板

![下载模板](./images/下载模板.gif)
...
![单点造数据](./images/单点造数据.gif)

#### 2.负载均衡消费数据

![负载均衡](./images/负载均衡.gif)

#### 3.查看队列数据

![查看队列数据](./images/查看队列数据.gif)

#### 4.ReplaceText为a,b,c,d

![ReplaceText后队列](./images/ReplaceText后队列.gif)

![image-20200216073534937](.\images\ReplaceText后队列.png)

#### 5.通过ExtractText将数据写入属性

设置新属性csv，通过正则表达式"(.+),(.+),(.+),(.+)"进行匹配后，将FlowFile的内容"a,b,c,d"写入名为csv的属性。

![ExtractText正则匹配](./images/ExtractText正则匹配.gif)

查看输出数据属性

![ExtractText输出](./images/ExtractText输出内容.gif)
...
![image-20200216084952967](.\images\支持表达式语言.png)

![image-20200216085258114](.\images\不支持表达式语言.png)
...
![使用表达式语言](./images/使用表达式语言设置属性.gif)



### 查看替换后的内容

![表达式替换后](./images/表达式替换后.gif)
...
![启动CsvToJson](./images/启动CsvToJson.gif)



### 状态栏

![image-20200216173718730](.\images\状态栏.png)

### 处理器面板

## NiFi提供有关画布上每个处理器的大量信息。

![img](.\images\processor-anatomy.2a654c02.png)
...
![img](./images/process-group-anatomy.png)
...
![img](.\images\summary-table.png)

此对话框提供有关画布上每个组件的大量信息。下面,我们在对话框中注释了不同的元素,以便更容易地讨论对话框。

![img](./images/summary-annotated.png)
...
![NifiSummary](./images/NifiSummary.gif)
...
![img](.\images\stats-history.d0427901.png)
...
对话框的右侧提供了下表中要呈现的不同类型度量标准的下拉列表。顶部图形较大,以便提供更容易阅读的信息呈现。在该图的右下角是一个小手柄(![img](.\images\img15324.png)),可以拖动它来调整图形的大小。也可以拖动对话框的空白区域以移动整个对话框。
...
![StatusHistory.gif](./images/StatusHistory.gif)
...
![img](.\images\provenance-annotated.594659f0.png)
...
![img](./images/search-receive-event-abc.png)
...
![img](./images/expanded-events.png)



#### 操作

![查看DataProvenance](./images/查看DataProvenance.gif)



## 连接与关系

将处理器和其他组件添加到画布并进行配置后,下一步是将它们彼此连接,以便让NiFi知道在处理完每个FlowFile后如何传输数据。这是通过在每个组件之间创建连接来实现的。当用户将鼠标悬停在组件的中心上时,会出现一个新的连接图标![img](./images/连接箭头.png)

![img](.\images\processor-connection-bubble.8828c836.png)
...
![img](.\images\create-connection.26dec2a6.png)

此外,此选项卡还提供了选择此Connection中应包含哪些关系的功能。必须至少选择一个关系。如果只有一个关系可用,则会自动选择它。

![img](./images/说明.png)如果使用相同的关系添加多个Connections,则路由到该关系的任何FlowFile将自动clone,并且将向每个Connections发送一个副本。

### 设置

设置选项卡提供配置连接名称,FlowFile到期,背压阈值,负载平衡策略和优先级的功能：

![img](./images/connection-settings.0dd12b66.png)
...
![img](.\images\file_expiration_clock.c2cf7431.png)
...
![img](./images/说明.png)默认情况下,添加的每个新连接都将具有默认的背压对象阈值10000 objects和背压数据大小阈值1 GB。可以通过修改nifi.properties文件中的相应属性来更改这些默认值：
...
![img](.\images\back_pressure_indicators.9e0cf252.png)

将鼠标悬停在条形图上会显示确切的百分比。

![img](.\images\back_pressure_indicator_hover.d43e6391.png)

队列完全填满后,Connection将以红色突出显示。

![img](.\images\back_pressure_full.1410d806.png)
...
![img](.\images\注意.png)NiFi会在重新启动时持久保存集群中的节点。这可以防止在所有节点都已连接之前重新分配数据。如果集群已关闭且不打算重新启动节点,则用户有责任通过UI中的集群对话框从集群中删除该节点。

##### 负载平衡压缩

选择负载平衡策略后,用户可以配置在集群中的节点之间传输时是否应压缩数据。

![img](.\images\load_balance_compression_options.7493ebb9.png)
...
![img](.\images\load_balance_configured_connection.b34f6db9.png)

将鼠标悬停在该图标上将显示连接的负载平衡策略和压缩配置。此状态下的图标还表示连接中的所有数据都已在集群中分布。

![img](.\images\load_balance_distributed_connection.2659a4a0.png)

当在集群中的节点之间主动传输数据时,负载平衡指示器将更改方向和颜色：

![img](.\images\load_balance_active_connection.af7bfb32.png)

##### 集群连接摘要

要查看在集群节点之间分配数据的位置,请从全局菜单中选择摘要。然后选择Connections选项卡和View Connection Details：

![img](.\images\summary_connections.2e8e404a.png)

这将打开集群连接摘要对话框,该对话框显示集群中每个节点上的数据：

![img](.\images\cluster_connection_summary.600a1374.png)
...
![img](./images/说明.png)配置了负载平衡策略后，连接除了本地队列外，每个集群节点都有一个队列。优先排序器将独立地对每个队列中的数据进行排序。

### 更改配置和上下文菜单选项

在两个组件之间建立连接之后，可以更改连接的配置，并且可以将连接移动到新目的地;但是，必须先停止连接任一侧的处理器，然后才能进行配置或目标更改。

![img](.\images\nifi-connection.ecf965b8.png)

要更改连接的配置或以其他方式与连接交互,请右键单击连接以打开连接上下文菜单。

![img](.\images\nifi-connection-menu.6a39f3e8.png)
